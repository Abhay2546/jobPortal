				...
				if (res.length == 0) {
					...
					for (var i = 0; i < res.length; i++) {
						...
						$('.results').append(
							...
							"					<div class='p0 job-salary col-xs-12 col-sm-6 col-md-4 col-lg-3'><i class='fa1 fa fa-rupee'></i>" + res[i].salary_min + " - " + res[i].salary_max + "</div>" +
	...
	$(document).ready(function() {
		...
		$(function() {
			$("#slider-range").slider({
				...
				min: <?= $salary_min ?>,
				max: <?= $salary_max ?>,
				values: [<?php if (isset($_GET['salary_min'])) {
								echo $salary_min = $_GET['salary_min'];
							} else {
								echo $salary_min;
							} ?>, <?php if (isset($_GET['salary_max'])) {
																																		echo $salary_max = $_GET['salary_max'];
																																	} else {
																																		echo $salary_max;
																																	} ?>],
				slide: function(event, ui) {
					...
					$(".sliderange").html("<i class='fa fa-rupee'></i> " + ui.values[0] + " - <i class='fa fa-rupee'></i> " + ui.values[1]);
					get_result();
				}
			});
			$("#salary_min").val($("#slider-range").slider("values", 0));
			$("#salary_max").val($("#slider-range").slider("values", 1));
			$(".sliderange").html("<i class='fa fa-rupee'></i> " + $("#slider-range").slider("values", 0) + " - <i class='fa fa-rupee'></i> " + $("#slider-range").slider("values", 1));
		});
